...
INPUT_VARIABLES = ["air_temperature", "attached_weight", "position_x", "position_y", "process_temperature", "speed", "speed_next"]
    ...
    def datachange_notification(self, node, val, data):
        ...
        if feature_name in self.feature_names:
            ...
            if self.current_step == 2:
                if all(self.latest_values.get(var) is not None for var in INPUT_VARIABLES):
                    print("Computing correction values.", flush=True)
                    self._compute_and_write_corrections(self.latest_values.copy(), self.cycle)  # Pass a copy to avoid mutation issues during runtime
                else:
                    print("Warning: Not all input variables were collected before step change.", flush=True)
                    print("Missing: ", [var for var in self.feature_names if self.latest_values.get(var) is None], flush=True)
...
def predict_error(input_data):
    ...
    feature_cols = ['Air temperature [K]', 'Attached Weight [mg]', 'Position x [mm]', 'Position y [mm]', 'Process temperature [K]', 'Speed [mm/s]', 'Speed next [mm/s]']
    ## <<<
